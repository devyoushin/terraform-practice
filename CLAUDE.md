...
두 가지 패턴을 모두 포함합니다:
- **레거시 패턴**: `ops/legacy/<module>/modules/` + `ops/legacy/<module>/envs/` 분리
- **Terragrunt 패턴**: `ops/dev/` + `ops/prod/` 환경 디렉토리 + 루트 `ops/terragrunt.hcl` (권장)
...
| `/review-doc` | 코드/문서 검토 | `/review-doc ops/legacy/eks/modules/eks/main.tf` |
...
- README 스타일 기준: `ops/legacy/eks/README.md` 참고
...
# 특정 모듈만 (+ 모든 의존성 포함)
terragrunt run-all apply --terragrunt-working-dir ops/dev/rds/
```
...
cd ops/dev/rds && terragrunt plan
```
