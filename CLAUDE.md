...
| Multi-AZ / 복제 | 단일 AZ | 단일 AZ | Multi-AZ |
| KMS 암호화 | 선택적 | 권장 | **필수** |
| CloudWatch 알람 | 비활성화 | 활성화 | 활성화 |
| 백업/보존 기간 | 최단 | 중간 | 최장 |
| `prevent_destroy` | 없음 | 없음 | **필수** |
...
- [ ] `envs/dev/`, `envs/staging/`, `envs/prod/` — 각각 `main.tf`, `variables.tf`, `terraform.tfvars`, `backend.tf`
...
| 모듈 | modules | envs(dev) | envs(staging/prod) | Makefile | pre-commit | README |
|------|---------|-----------|---------------------|----------|------------|--------|
