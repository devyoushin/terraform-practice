...
variable "subnet_ids" {
  description = "RDS 서브넷 그룹에 포함될 프라이빗 서브넷 ID 목록 (Multi-AZ를 위해 최소 3개, 가용 영역 분산 필수)"
  type        = list(string)
}
...
variable "db_password" {
  description = "DB 마스터 비밀번호 (민감 정보 - 반드시 AWS Secrets Manager 또는 환경변수로 주입, tfvars 직접 작성 금지)"
  type        = string
  sensitive   = true
}
...
variable "db_instance_class" {
  description = "RDS 인스턴스 클래스 (prod 기본값: db.t3.medium, 워크로드에 따라 상향 조정)"
  type        = string
