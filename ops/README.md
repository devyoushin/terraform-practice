...
| Landing Zone foundation | `landing-zone/` | AWS Organizations, OU, 계정 분리, 서비스별 CIDR 계획 |
...
├── landing-zone/               # Organizations, 계정, 50개 서비스 CIDR 설계
│   ├── catalog/                 # OU/account/service IP 카탈로그
│   ├── envs/foundation/         # Landing Zone foundation 실행 경로
│   └── modules/                 # Organizations 및 CIDR 계획 모듈
├── envs/                       # 환경 기준값 참조 문서
│   ├── dev.hcl
│   └── prod.hcl
...
| `landing-zone/` | 조직/계정/OU/IP 대역 foundation 구성 | 서비스별 애플리케이션 리소스 |
...
`landing-zone/`은 기존 서비스 `live/`와 별도 state로 운영한다. 먼저 Control Tower 랜딩존 상태와 계정 생성 정책을 확인한 뒤 실행한다.
...
기본 설계는 50개 서비스마다 `dev`, `stg`, `prd` 계정을 분리하고, `10.100.0.0/16`에서 서비스별 `/24`, 환경별 `/26`을 할당한다.
