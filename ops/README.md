...
| `checklists/` | apply 전 점검, 모듈 리뷰 체크리스트 |
| `runbooks/` | drift 점검, state import 등 운영 절차 |
...
- `prod` apply 전에는 `checklists/pre-apply.md`를 먼저 확인합니다.
- 새 모듈이나 큰 변경은 `checklists/module-review.md` 기준으로 리뷰합니다.
- 예상하지 못한 변경이 보이면 `runbooks/drift-detection.md` 흐름으로 원인을 분리합니다.
- 기존 리소스를 Terraform 관리 대상으로 편입할 때는 `runbooks/state-import.md`를 사용합니다.
